...
static void osdDrawLogo(int x, int y)
{
    ...
    if (checkBeesignSerialPort()) {
        ...
        for (column = 0; column < 6; column++) {
            displayWriteChar(osdDisplayPort, x + column, y, DISPLAYPORT_ATTR_NONE, fontOffset++);
        }
        for (column = 7; column < 24; column++) {
            displayWriteChar(osdDisplayPort, x + column, y, DISPLAYPORT_ATTR_NONE, 0xC0);
        }
        for (column = 0; column < 24; column++) {
            displayWriteChar(osdDisplayPort, x + column, y + 1, DISPLAYPORT_ATTR_NONE, fontOffset++);
        }
        for (column = 0; column < 24; column++) {
            displayWriteChar(osdDisplayPort, x + column, y + 2, DISPLAYPORT_ATTR_NONE, fontOffset++);
        }
        for (column = 0; column < 6; column++) {
            displayWriteChar(osdDisplayPort, x + column, y + 3, DISPLAYPORT_ATTR_NONE, fontOffset++);
        }
        for (column = 7; column < 24; column++) {
            displayWriteChar(osdDisplayPort, x + column, y + 3, DISPLAYPORT_ATTR_NONE, 0xC0);
        }
    } else
#endif
    {
...
#ifdef USE_OSD_BEESIGN
    displayWrite(osdDisplayPort, 18, 5, DISPLAYPORT_ATTR_NONE, string_buffer);
#else
...
#ifdef USE_CMS
#ifdef USE_OSD_BEESIGN
    displayWrite(osdDisplayPort, 7, 7, DISPLAYPORT_ATTR_NONE, CMS_STARTUP_HELP_TEXT1);
    displayWrite(osdDisplayPort, 11, 8, DISPLAYPORT_ATTR_NONE, CMS_STARTUP_HELP_TEXT2);
    displayWrite(osdDisplayPort, 11, 9, DISPLAYPORT_ATTR_NONE, CMS_STARTUP_HELP_TEXT3);
#else
...
{
#ifdef USE_OSD_BEESIGN
    displayWrite(osdDisplayPort, 0, y, DISPLAYPORT_ATTR_NONE, text);
    displayWrite(osdDisplayPort, 17, y, DISPLAYPORT_ATTR_NONE, ":");
    displayWrite(osdDisplayPort, 19, y, DISPLAYPORT_ATTR_NONE, value);
#else
...
#ifdef USE_OSD_BEESIGN
        displayWrite(osdDisplayPort, 0, displayRow, DISPLAYPORT_ATTR_NONE, buff);
#else
...
static bool osdRenderStatsContinue(void)
{
    if (osdStatsRenderingState.row == 0) {
        ...
        if (displayLabel) {
#ifdef USE_OSD_BEESIGN
            displayWrite(osdDisplayPort, 0, osdStatsRenderingState.row++, DISPLAYPORT_ATTR_NONE, "  --- STATS ---");
#else
...
#ifdef USE_OSD_BEESIGN
    displayWrite(osdDisplayPort, 9, 4, DISPLAYPORT_ATTR_NONE, "ARMED");
#else
